<div class="relative inline-block text-left">
    <div id="menu-button">
        <button type="button" id="profile-button" class="inline-flex w-full justify-center gap-x-1.5 px-3 text-sm font-semibold text-blue-300 underline" aria-expanded="true" aria-haspopup="true">
            {{ auth()->user()->name }}
    ...
    -->
    <div id="profile-menu" class="absolute hidden right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-neutral-900 border border-gray-500 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
...
{{--        </div>--}}
        <div class="py-1" role="none">
            <form action="/logout" method="POST" class="w-full">
                @csrf
                <button type="submit" class="w-full block px-4 py-2 text-sm text-red-400 hover:bg-red-800/40" role="menuitem" tabindex="-1" id="menu-item-6">Выйти</button>
            </form>
        </div>
    </div>
</div>
