<tr {{ $attributes->merge(['class' => "text-gray-950 " . ($task->isParent() ? 'bg-neutral-300' : 'bg-neutral-50')]) }}>
    ...
    <td class="flex gap-1.5 justify-center items-center m-5 mr-2 ml-1">
        ...
        @if($task->manager_id === auth()->user()->id || (auth()->user()->role->name === 'head-of-department' && auth()->user()->department_id === $task->department_id))
